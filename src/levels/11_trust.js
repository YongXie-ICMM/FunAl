FunAl.register({
  ...
  async run(t) {
    ...
    const BP = B.bigPenalty;
    await t.ask.predict(`那把罚款调大呢？假设不是罚 ${B.penalty} 元，而是**罚 ${BP.penalty} 元**。最省的路线还会是这一条吗？`, [
      '还是这一条，罚款多少不影响哪条路最省',
      '会换成别的路线',
    ]);
    await t.say(`会换。罚 ${BP.penalty} 元的话，原来那条 ${D.bestPath.join('→')} 要付 **${BP.oldRouteNowCosts} 元**（${B.realBreakdown.toll} 元过路费加 ${BP.penalty} 元罚款），贵得离谱。`);
    await t.say(`这时候最省的变成 **${BP.paths[0].join('→')}** 这类路线，一共 **${BP.best} 元**——它们的共同点是**一次「连着跨两次 2 级」都没有**，一分罚款都不用付。`);
    await t.say('所以别觉得老表运气好。**一张算出假价钱的表，你没有理由相信它指的路是真的。**');
